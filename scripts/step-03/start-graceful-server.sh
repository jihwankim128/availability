...
if [[ -f "${PID_FILE}" ]] && kill -0 "$(<"${PID_FILE}")" 2>/dev/null; then
  echo "Graceful server is already running. PID=$(<"${PID_FILE}")"
  exit 1
fi

RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
RESULT_DIR="${PROJECT_ROOT}/results/step-03/${RUN_ID}"
LOG_FILE="${RESULT_DIR}/server.log"
mkdir -p "${RESULT_DIR}"
echo "${RUN_ID}" >"${RUN_ID_FILE}"

nohup env APP_VERSION=v1 INSTANCE_ID=graceful \
  java -jar "${JAR_FILE}" --spring.profiles.active=graceful \
  >"${LOG_FILE}" 2>&1 &
...
for _ in {1..30}; do
  ...
  if [[ "${SERVER_INFO}" == *'"instanceId":"graceful"'* ]]; then
    echo "Graceful server started. PID=${SERVER_PID}"
    echo "Run ID: ${RUN_ID}"
    echo "Log: ${LOG_FILE}"
    exit 0
  fi
