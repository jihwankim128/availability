...
if [[ ! -f "${RUN_ID_FILE}" ]]; then
  echo "Run ID file does not exist. Start the graceful server first."
  exit 1
fi
...
echo "Grafana dashboard: http://localhost:3000/d/step-03-graceful-shutdown"
echo "Run ID: ${RUN_ID}"
...
RUN_ID="${RUN_ID}" \
  ...
  k6/step-03-graceful-shutdown.js
