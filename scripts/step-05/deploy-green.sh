...
if [[ ! -f "${BLUE_PID_FILE}" ]] || [[ ! -f "${RUN_ID_FILE}" ]]; then
  echo "Step 5 runtime files do not exist. Start Blue first."
  exit 1
fi
...
nohup env APP_VERSION=v2 INSTANCE_ID=green \
  java -jar "${JAR_FILE}" --spring.profiles.active=blue-green --server.port=8082 \
  >"${GREEN_LOG_FILE}" 2>&1 &
