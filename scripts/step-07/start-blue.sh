...
COMPOSE_FILE="${PROJECT_ROOT}/deploy/step-07/docker-compose.yml"
NGINX_TEMPLATE="${PROJECT_ROOT}/deploy/step-07/nginx/default.conf.template"
...
for PID_FILE in "${BLUE_PID_FILE}" "${GREEN_PID_FILE}"; do
  if [[ -f "${PID_FILE}" ]] && kill -0 "$(<"${PID_FILE}")" 2>/dev/null; then
    echo "Step 7 server is already running. PID=$(<"${PID_FILE}")"
    exit 1
  fi
done
...
if nc -z localhost 6379 2>/dev/null; then
  echo "Port 6379 is already in use. Stop the existing Redis first."
  exit 1
fi
...
nohup env APP_VERSION=v1 INSTANCE_ID=blue \
  java -jar "${JAR_FILE}" \
  --spring.profiles.active="blue-green,session-redis" \
  --server.port=8081 \
  >"${BLUE_LOG_FILE}" 2>&1 &
...
BLUE_READINESS="$(curl -fsS http://localhost:8081/actuator/health/readiness 2>/dev/null || true)"
if [[ "${BLUE_READINESS}" != *'"status":"UP"'* ]]; then
...
for _ in {1..30}; do
  ...
  if [[ "${PROXY_INFO}" == *'"version":"v1"'* ]] && [[ "${PROXY_INFO}" == *'"instanceId":"blue"'* ]]; then
    echo "Blue v1 started with Redis-backed sessions. PID=${BLUE_PID}"
    echo "Run ID: ${RUN_ID}"
