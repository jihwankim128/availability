...
RUNTIME_DIR="${PROJECT_ROOT}/build/external-api/step-04"
RUN_ID_FILE="${RUNTIME_DIR}/run-id"
COMPOSE_FILE="${PROJECT_ROOT}/deploy/external-api/step-04/docker-compose.yml"

if [[ ! -f "${RUN_ID_FILE}" ]]; then
  echo "Run ID file does not exist. Start External API Step 4 first."
  exit 1
fi

if [[ -z "$(docker compose -f "${COMPOSE_FILE}" ps -q application)" ]]; then
  echo "External API Step 4 application is not running. Run: bash scripts/external-api/step-04/start.sh"
  exit 1
fi
...
if ! curl -fsS http://localhost:8080/actuator/health/readiness >/dev/null; then
  echo "Application is not ready. Run: bash scripts/external-api/step-04/start.sh"
  exit 1
fi

RUN_ID="$(<"${RUN_ID_FILE}")"
CLIENT_TIMEOUT="${CLIENT_TIMEOUT:-3s}"
RESULT_DIR="${PROJECT_ROOT}/results/external-api/step-04/${RUN_ID}"
...
echo "Run ID: ${RUN_ID}"
echo "Timeline: 0~10s normal 10 RPS, 10~50s 5s delay at 50 RPS, 50~70s recovery 10 RPS"
echo "Independent /api/info load: 2 RPS for 70 seconds"
echo "Server Read Timeout: 1s"
echo "k6 Client Timeout: ${CLIENT_TIMEOUT}"
echo "Grafana: http://localhost:3000/d/external-api-step-04-timeout-limit"
...
CLIENT_TIMEOUT="${CLIENT_TIMEOUT}" \
  ...
  k6/external-api/step-04-timeout-limit.js
