...
if [[ ! -f "${RUN_ID_FILE}" ]]; then
  echo "Run ID file does not exist. Start External API Step 5 first."
  exit 1
fi

if [[ -z "$(docker compose -f "${COMPOSE_FILE}" ps -q application)" ]]; then
  echo "External API Step 5 application is not running. Run: bash scripts/external-api/step-05/start.sh"
  exit 1
fi
...
if ! curl -fsS http://localhost:8080/actuator/health/readiness >/dev/null; then
  echo "Application is not ready. Run: bash scripts/external-api/step-05/start.sh"
  exit 1
fi
...
echo "Grafana: http://localhost:3000/d/external-api-step-05-circuit-breaker"
...
CLIENT_TIMEOUT="${CLIENT_TIMEOUT}" \
  ...
  k6/external-api/step-05-circuit-breaker.js
