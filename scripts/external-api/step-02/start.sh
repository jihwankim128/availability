...
for _ in {1..60}; do
  ...
  if [[ "${READINESS}" == *'"status":"UP"'* ]] && \
    ...
    echo "External API Step 2 application, WireMock, and TCP blackhole started."
    echo "Run ID: ${RUN_ID}"
    echo "Application: http://localhost:8080"
    echo "WireMock: http://localhost:9091"
    echo "Connect Timeout: 1 second"
...
echo "External API Step 2 application did not become ready. Check ${SERVER_LOG_FILE}"
exit 1
