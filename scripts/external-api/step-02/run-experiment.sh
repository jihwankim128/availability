...
RUNTIME_DIR="${PROJECT_ROOT}/build/external-api/step-02"
RUN_ID_FILE="${RUNTIME_DIR}/run-id"
COMPOSE_FILE="${PROJECT_ROOT}/deploy/external-api/step-02/docker-compose.yml"

if [[ ! -f "${RUN_ID_FILE}" ]]; then
  echo "Run ID file does not exist. Start External API Step 2 first."
  exit 1
fi

if [[ -z "$(docker compose -f "${COMPOSE_FILE}" ps -q application)" ]]; then
  echo "External API Step 2 application is not running. Run: bash scripts/external-api/step-02/start.sh"
  exit 1
fi
...
if ! curl -fsS http://localhost:8080/actuator/health/readiness >/dev/null; then
  echo "Application is not ready. Run: bash scripts/external-api/step-02/start.sh"
  exit 1
fi

RUN_ID="$(<"${RUN_ID_FILE}")"
CLIENT_TIMEOUT="${CLIENT_TIMEOUT:-3s}"
RESULT_DIR="${PROJECT_ROOT}/results/external-api/step-02/${RUN_ID}"
...
echo "Run ID: ${RUN_ID}"
echo "Timeline: 0~15s normal, 15~40s TCP connection blackhole, 40~60s recovery"
echo "Load: external API 10 RPS + independent /api/info 2 RPS"
echo "Server Connect Timeout: 1s"
echo "k6 Client Timeout: ${CLIENT_TIMEOUT}"
echo "Grafana: http://localhost:3000/d/external-api-step-02-connect-timeout"
...
CLIENT_TIMEOUT="${CLIENT_TIMEOUT}" \
  ...
  k6/external-api/step-02-connect-timeout.js
