...
const externalReadTimeoutResponse = new Counter('external_read_timeout_response');
...
const externalReadFaultSignal = new Gauge('external_read_fault_signal');
...
export const options = {
  ...
  thresholds: {
    ...
    external_read_timeout_response: ['count>0'],
...
function arrivalRateScenario(execName, rate, duration) {
  return {
...
export function requestFeaturedProductByPhase() {
  ...
  const totalExternalRequests = requestRate * totalDurationSeconds;
  if (iteration >= totalExternalRequests) {
    return;
  }

  const faultStartsAt = requestRate * healthySeconds;
  const recoveryStartsAt = requestRate * (healthySeconds + faultSeconds);
  const faulted = iteration >= faultStartsAt && iteration < recoveryStartsAt;

  if (iteration === 0) {
    setExternalMode('/__lab/control/normal');
  } else if (iteration === faultStartsAt) {
    setExternalMode('/__lab/control/slow');
  } else if (iteration === recoveryStartsAt) {
    setExternalMode('/__lab/control/normal');
  }

  const phase = faulted ? 'read_failure' : 'normal';
  const response = http.get(`${baseUrl}/api/external/featured-product`, {
    tags: {
      experiment: 'external-api-step-03-read-timeout',
  ...
  externalReadFaultSignal.add(faulted ? 1 : 0);
  externalUserDuration.add(response.timings.duration, { phase });

  if (faulted) {
    externalReadTimeoutResponse.add(response.status === 504 ? 1 : 0);
    externalClientTimeout.add(response.status === 0 ? 1 : 0);
    externalFailureDuration.add(response.timings.duration);

    check(response, {
      '서버가 Read Timeout 후 504 응답': (result) => result.status === 504,
